...
import ax.clio.memory.code.PgVectorCodeChunkVectorSearch;
...
/**
 * 프로덕션 이슈 벡터검색: pgvector 코사인 거리 {@code <=>} top-k (I4/I7). {@code embedding}은 이식적
 * {@code real[]} 컬럼이라 쿼리 시 {@code ::vector}로 캐스팅한다. pgvector는 H2에 없어 <b>CI 미실행</b>이며
 * 실경로 검증은 별도 벤치마크의 몫이다. #7 {@link PgVectorCodeChunkVectorSearch}의 병렬 구현.
 */
@Component
public class PgVectorIssueVectorSearch implements IssueVectorSearch {
	...
	public PgVectorIssueVectorSearch(IssueEmbeddingRepository issueEmbeddingRepository) {
	...
	public List<ScoredIssue> searchByVector(Long projectId, float[] query, int topK) {
		...
		Map<Long, IssueEmbedding> issueById = new LinkedHashMap<>();
		issueEmbeddingRepository.findAllById(scoreById.keySet())
				.forEach(issue -> issueById.put(issue.getId(), issue));

		return scoreById.entrySet().stream()
				.filter(entry -> issueById.containsKey(entry.getKey()))
				.map(entry -> new ScoredIssue(issueById.get(entry.getKey()), entry.getValue()))
				.toList();
	}

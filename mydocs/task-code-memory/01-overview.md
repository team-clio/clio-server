...
동작했다. 이 작업은 코드를 **chunk 단위로 저장하고, chunk별 embedding을 만들어**, 의미(semantic)
검색을 **필요할 때 부르는 도구**로 추가한다. roadmap "추천 개발 순서" #7 (3.1 Code Memory).

목표(roadmap 3.1):

> 버그 리포트와 **의미적으로 가까운 코드**를 찾을 수 있어야 한다.

> **방향 재정의(D0-A, `03-decisions.md`)**: "코드에선 agentic search가 RAG보다 낫다"는 지적을 반영해,
> 이 작업은 **풀 RAG 파이프라인이 아니다.** 주 경로는 키워드+구조 검색(에이전트 오케스트레이션·MCP 준비)이고,
> semantic search는 키워드가 헛발질할 때(사용자 명사 ≠ 코드 명사) **필요할 때 부르는 fallback/도구**다.
> "항상 블렌딩"이 아니라 "필요할 때 호출".
>
> **범용 substrate(D0-B)**: embed+저장+유사도검색을 "코드 전용"이 아니라 임의 텍스트에 재사용 가능하게 설계한다.
> "쓸수록 좋아지는" 학습 루프(과거 리포트↔실제 수정 위치)는 **피드백 신호가 필요**하므로 #8 Issue Memory로 미룬다.
> 지금은 그 레일만 깐다.
...
### D. 하이브리드 검색 (재정의: 필요할 때 호출)
키워드+구조 검색이 **주 경로**다. semantic 검색은 키워드가 빈약할 때의 **fallback/도구**로만 붙인다
(항상 블렌딩 아님). 분석 파이프라인(`CodeCandidateRanker`/`AnalysisWorker`)과 어떻게 붙일지는 결정 포인트(D6).

## 범위

포함(예정): 코드 chunk 저장, chunk embedding 생성, 벡터 유사도 검색(`semanticSearch`),
**필요할 때 호출하는** 하이브리드(키워드 주 + semantic fallback)의 최소 동작 경로.
substrate는 임의 텍스트에 재사용 가능하게 설계(D0-B).

제외: Issue Memory(#8) / Decision Memory(#9), **"쓸수록 좋아지는" 학습 루프·outcome 캡처(#8)**,
      LLM 요약(#10), chunk 재랭킹 정교화(추후 튜닝),

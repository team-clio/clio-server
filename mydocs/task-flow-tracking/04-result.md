...
## 커밋 구성 (작업 단위별)
```
docs: overview / plan / decisions(F1~F9)
feat: CodeFlow/FlowNode 모델
feat: CodeDependencyGraph (+test 6)
feat: FlowTracer (+test 7)
feat: 파이프라인 연결 + 결과 구조화 + 점수 반영
docs: result + 진행 상태 갱신
```
...
## 점수 반영 (가벼움)
- 난이도: 흐름이 걸친 distinct 레이어 수 × 5 가산(넓게 걸칠수록 수정 범위 큼). 값은 잠정.
...
## 남은 과제 (후속)
- **D9-B(F9)**: 흐름 내 프로덕션 클래스의 대응 테스트 존재 여부를 그래프로 판정해 결과에 표시.

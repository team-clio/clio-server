...
## P7. 규칙 문서화 위치
**선택: `mydocs/workflow-rules.md` 에 "패키지 규칙" 절 추가.**
**이유**: 이미 "절대 준수" 권위 문서이고 **작업 시작 전 반드시 읽는** 문서라 실제로 지켜질 확률이 가장 높다.
루트 `CLAUDE.md` 신설은 권위 문서를 둘로 갈라 규칙이 어긋날 여지를 만든다(이번 작업이 바로 문서와 코드가
어긋나서 생긴 일). `package-info.java` 분산은 전체 규칙이 한 번에 안 읽힌다.
